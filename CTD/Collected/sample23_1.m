...
for jj=1:size(data.PT,2) 
    ...
    PT_tmp =squeeze(data.PT(ii,jj,q)); %potential temperature at ii-th layer and jj-th station

    [R,p]  =corrcoef([PT_tmp,PDO]);
    PT_PDO_R(ii,jj) =R(1,2); %Correlation coefficient
    PT_PDO_p(ii,jj) =p(1,2); %p-value
...
contourf(data.lat,[0:1000],PT_PDO_R,20,'edgecolor','none');
xlabel('Latitude [deg N]')
ylabel('Pressure [dbar]')
set(gca,'ylim',[0,1000],'xdir','reverse','Fontsize',20)
colormap(redblue); 
cl=colorbar; 
ylabel(cl,'Corr Coeff between potential temperature and PDO')
caxis([-0.5,0.5]); 
axis ij; 
hold on

%Contour line for showing p-value <0.05
contour(data.lat,[0:1000],PT_PDO_p,[0, 0.05],'y-','linewidth',3)
