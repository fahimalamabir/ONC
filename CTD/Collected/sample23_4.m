...
for jj=1:size(data.S,2)
    ...
    PT_tmp =squeeze(data.S(ii,jj,q)); %potential temperature at ii-th layer and jj-th station
    [R,p]  =corrcoef([PT_tmp,NPGO]);
    PT_NPGO_R(ii,jj) =R(1,2); %Correlation coefficient
    PT_NPGO_p(ii,jj) =p(1,2); %p-value
    
end
end

%plot sectional distribution of correlation between salinity and NPGO
figure('Position',[0,0,1200,800])
contourf(data.lat,[0:1000],PT_NPGO_R,20,'edgecolor','none');
xlabel('Latitude [deg N]')
ylabel('Pressure [dbar]')
set(gca,'ylim',[0,1000],'xdir','reverse','Fontsize',20)
colormap(redblue); 
cl=colorbar; 
ylabel(cl,'Corr Coeff between potential temperature and NPGO 2-yr Lag')
caxis([-0.5,0.5]); 
axis ij; 
hold on
...
contour(data.lat,[0:1000],PT_NPGO_p,[0, 0.05],'y-','linewidth',3)
