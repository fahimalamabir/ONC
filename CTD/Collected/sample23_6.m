...
for jj=1:size(data.vel_1000,2) 
    ...
    [R,p]  =corrcoef([vel_tmp,PDO]);
    vel_PDO_R(ii,jj) =R(1,2); %Correlation coefficient
    vel_PDO_p(ii,jj) =p(1,2); %p-value
    
end
end

%plot sectional distribution of correlation between geostrophic velocity and PDO
figure('Position',[0,0,1200,800])
contourf(lat,[0:1000],vel_PDO_R,20,'edgecolor','none');
xlabel('Latitude [deg N]')
ylabel('Pressure [dbar]')
set(gca,'ylim',[0,1000],'xdir','reverse','Fontsize',20)
colormap(redblue); 
cl=colorbar; 
ylabel(cl,'Corr Coeff between Geostrophic Vel and PDO')
caxis([-0.5,0.5]); 
axis ij; 
hold on
...
contour(lat,[0:1000],vel_PDO_p,[0, 0.05],'y-','linewidth',3)
